...
<template>
  ...
  <div>
    ...
    <!-- <el-button
      class="btn"
      style="position: fixed; right: 5%; bottom: 20%; "
    >
      <router-link style="text-decoration : none;color: #000;" to="contract">合同解析服务</router-link>
    </el-button> -->
    <div class="font"><span>简历解析服务</span></div>
    <div class="tip">
      <!-- <pre> -->
          使用说明：把需要解析的简历pdf按部门文件夹分类，统一放在一个叫 root 的文件夹,压缩成 root.zip 上传。
    ...
    async download() {
      try {
        ...
        if (response.status === 200) {
          ...
          link.href = url;
          link.download = "analysis_resume_result.zip"; // 设置下载文件名
          link.click();
